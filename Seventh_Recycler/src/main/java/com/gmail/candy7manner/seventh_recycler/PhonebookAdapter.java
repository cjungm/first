...
//      Adapter 객체 정의
//      데이터(Phonebook) 을 받아서 Item 별로 View 를 생성하는 역할
    public class PhonebookAdapter extends RecyclerView.Adapter<PhonebookAdapter.ViewHolder> {
        ...
        public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            ...
            // 준비된 레잉아웃(XML) 로부터 View를 만들어 ViewGroup 에 붙이고
            // 만들은 View를 리턴한다
        ...
        public int getItemCount() {
            return items.size();  // ArrayList<> 의 size()
        }

        // Adapter 안에 ViewHolder 내부클래스 생성
        static class ViewHolder extends RecyclerView.ViewHolder {

            // ViewHolder 안에 담긴 각각의 View를 담을 변수
            ImageView ivPhoto;
            ...
            public ViewHolder(@NonNull View itemView) {
                ...
                itemView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        int position = getAdapterPosition();  // 클릭된 item 의 position 값
                        //Toast.makeText(view.getContext(), "position: " + position, Toast.LENGTH_LONG).show();

                        // 아이템을 클릭하면 해당 세부 정보 액티비티로 넘겨주기
                        Intent intent = new Intent(view.getContext(), PhonebookContents.class);
                ...
                // ViewHolder 에 담긴 View 들에 대한 이벤트 예제
                ...
                btnDelItem.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        // 삭제를 하려면 position 정보 필요하다
                        // getAdapterPosition() 사용

                        adapter.removeItem(getAdapterPosition());
                        // Adapter 모델에선, 데이터가 변경(추가, 삭제 포함) 되면
                        // 반드시 Adapter 에 변경을 알려주어(notify) Adapter 가
                        // 리스트 화면을 업데이트 하도록 해야 한다.
