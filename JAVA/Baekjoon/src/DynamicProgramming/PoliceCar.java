...
//		예를 들어 앞의 그림처럼 N=6인 경우, 
//		처리해야 하는 사건들이 3개 있고 그 사건들이 발생된 위치 를 순서대로 (3, 5), (5, 5), (2, 3)이라고 하자. 
//		(3, 5)의 사건을 경찰차2에 맡기고 (5, 5)의 사건도 경찰차2에 맡기며, 
//		(2, 3)의 사건을 경찰차1에 맡기면 두 차가 이동한 거리의 합은 4 + 2 + 3 = 9가 되 고, 더 이상 줄일 수는 없다.
...
//		두 사건이 발생한 위치가 같을 수 있다.
		...
		while(count>i) {
			int cost1 = Math.abs(car1[0]-accident[i][0])+Math.abs(car1[1]-accident[i][1]);
			int cost2 = Math.abs(car2[0]-accident[i][0])+Math.abs(car2[1]-accident[i][1]);
			if(cost1>cost2) {
				total+=cost2;
				car2[0]=accident[i][0];
				car2[1]=accident[i][1];
